...
class ReportRequest extends StatelessWidget {
  ReportRequest({super.key});

  final List<ReportModel> reports = [
    ReportModel(
      reportText: "Report #1",
      reportDate: DateTime.now().subtract(Duration(days: 1)),
      responseText: "GamePlan response to Report #1",
      responseDate: DateTime.now().subtract(Duration(hours: 20)),
    ),
    ReportModel(
      reportText: "Report #2",
    ...
    return Stack(
      children: [
        Padding(
          ...
          child:
              provider.noReportYet
                  ? loadEmptyView(
                    emptyIcon: Icons.report_outlined,
                    title: "You Haven't Reported Anything",
                    message:
                        "Read our Community Guidelines to learn what we allow on GamePlan and how you can help us report and remove what we don't.",
                    buttonTitle: "See Community Guidelines",
                    onPressed: () {
                      provider.setIsReportYet(false);
                    },
                  )
                  : ListView.builder(
